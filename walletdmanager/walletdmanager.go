...
func RequestAvailableBalanceToBeSpent(transferFeeString string) (availableBalance float64, err error) {
	...
	transferFee, err := strconv.ParseFloat(transferFeeString, 64) // transferFee is expressed in WEAK
	if err != nil {
...
func SendTransaction(transferAddress string, transferAmountString string, transferPaymentID string, transferFeeString string) (transactionHash string, err error) {
	...
	if !strings.HasPrefix(transferAddress, "WEAK") || (len(transferAddress) != 98 && len(transferAddress) != 187) {
	...
	transferAmount, err := strconv.ParseFloat(transferAmountString, 64) // transferAmount is expressed in WEAK
	if err != nil {
		return "", errors.New("amount is invalid")
	}

	if transferAmount <= 0 {
		return "", errors.New("amount of WEAK to be sent should be greater than 0")
	}
...
func StartWalletd(walletPath string, walletPassword string, useRemoteNode bool, useCheckpoints bool, daemonAddress string, daemonPort string) (err error) {
	...
	if isPlatformDarwin {
		...
		pathToAppLibDir := pathToHomeDir + "/Library/Application Support/AnonCoin-Nest"
